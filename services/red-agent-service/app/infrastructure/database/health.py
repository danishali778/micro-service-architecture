...
class DatabaseReadinessChecker:
    ...
    def check(self) -> ReadinessResult:
        if not self._internal_auth_validator.is_ready or not self._agent.is_ready:
            return ReadinessResult(
                ready=False,
                code="agent_or_auth_unconfigured",
                message="Red agent adapter or internal authentication is not configured.",
            )
        ...
        try:
            ...
            return ReadinessResult(
                ...
                message="The red agent database is unavailable.",
            )
        ...
        if self._settings.require_current_migration and version != CURRENT_MIGRATION_REVISION:
            return ReadinessResult(
                ...
                message="The red agent database migration is not current.",
            )
        return ReadinessResult(ready=True)
