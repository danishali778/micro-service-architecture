...
class DatabaseReadinessChecker:
    ...
    def check(self) -> ReadinessResult:
        if not self._internal_auth_validator.is_ready:
            return ReadinessResult(
                ready=False,
                code="internal_auth_unconfigured",
                message="Internal workload authentication is not configured.",
            )
        ...
        try:
            ...
            return ReadinessResult(
                ...
                message="The match orchestrator database is unavailable.",
            )
        ...
        if self._settings.require_current_migration and version != CURRENT_MIGRATION_REVISION:
            return ReadinessResult(
                ...
                message="The match orchestrator database migration is not current.",
            )
        return ReadinessResult(ready=True)
