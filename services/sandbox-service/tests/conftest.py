...
def make_test_settings(**overrides: object) -> Settings:
    values: dict[str, object] = {
        "environment": "test",
        "database_url": "postgresql+psycopg://sandbox:sandbox@127.0.0.1:5432/sandbox",
...
def auth_headers(
    *,
    ...
    scopes: str = "sandboxes:provision sandboxes:read sandboxes:terminate",
...
def provision_payload(match_id: str = "match_123") -> dict[str, object]:
    return {
        "match_id": match_id,
        "scenario": {
...
@pytest.fixture()
def repository(engine: Engine) -> SqlAlchemySandboxRepository:
    return SqlAlchemySandboxRepository(create_session_factory(engine))


@pytest.fixture()
def client(repository: SqlAlchemySandboxRepository) -> Iterator[TestClient]:
    app = create_app(
        settings=make_test_settings(),
        sandbox_repository=repository,
        provider=LocalFakeSandboxProvider(),
